...
describe('simple merkle tree', () => {
  for (const opts of [{}, { sortLeaves: true }, { sortLeaves: false }, { nodeHash: reverseHashPair }]) {
    describe(`with options '${JSON.stringify(opts)}'`, () => {
      ...
      it('generates valid single proofs for all leaves', () => {
        for (const [id, leaf] of tree.entries()) {
          ...
          assert(SimpleMerkleTree.verify(tree.root, leaf, proof1, opts.nodeHash));
        }
      });
      ...
      it('rejects invalid proofs', () => {
        ...
        assert(!SimpleMerkleTree.verify(tree.root, leaf, invalidProof, opts.nodeHash));
      });
      ...
      it('generates valid multiproofs', () => {
        for (const ids of [[], [0, 1], [0, 1, 5], [1, 3, 4, 5], [0, 2, 4, 5], [0, 1, 2, 3, 4, 5], [4, 1, 5, 0, 2]]) {
          ...
          assert(SimpleMerkleTree.verifyMultiProof(tree.root, proof1, opts.nodeHash));
        }
      });
      ...
      it('rejects invalid multiproofs', () => {
        ...
        assert(!SimpleMerkleTree.verifyMultiProof(tree.root, multiProof, opts.nodeHash));
      });

      it('renders tree representation', () => {
        const expected = // standard hash + unsorted
          (
            !opts.nodeHash && opts.sortLeaves === false
              ? [
  ...
  describe('tree dumps', () => {
    ...
    it('reject standard tree dump with a custom hash', () => {
      assert.throws(
        ...
        /^InvalidArgumentError: Data does not expect a custom node hashing function$/,
      );
    });

    it('reject custom tree dump without a custom hash', () => {
      assert.throws(
        () => SimpleMerkleTree.load({ format: 'simple-v1', hash: 'custom' } as any),
        /^InvalidArgumentError: Data expects a custom node hashing function$/,
      );
    });
